...
\begin{enumerate}
    ...
    \item \textbf{Mô-đun Đánh giá Chất lượng Câu hỏi Tự động (Evaluation Module):} Yêu cầu bắt buộc đối với các hệ thống sinh câu hỏi tự động là cần có các tiêu chuẩn kiểm định chất lượng đầu ra. Để có thể kiểm tra và đánh giá dữ liệu đầu ra một cách hoàn toàn tự động, % [MODIFY]: Loai bo metric Clarity, con lai ba tieu chi danh gia
nghiên cứu này sử dụng kỹ thuật đánh giá dựa trên LLM (LLM-as-a-judge). Nghiên cứu sử dụng bộ khung đánh giá gồm ba tiêu chí sư phạm: Khả năng giải quyết được dựa trên ngữ cảnh (Solvability); Chất lượng của phương án nhiễu (Distractor Quality) và Sự đáp ứng thang đo nhận thức (Alignment).
...
Nhằm giải quyết giới hạn không gian ngữ cảnh của phương pháp chỉ dẫn đơn, nghiên cứu đề xuất kiến trúc đa tác tử theo mô hình Trục-và-Nan hoa kết hợp Phân tán động (Hub-and-Spoke with Dynamic Fan-out). Hệ thống sử dụng khung điều phối LangGraph để xây dựng một đồ thị có hướng, trong đó một Nút Điều phối trung tâm đóng vai trò phân phối tác vụ tới các tác tử chuyên biệt. Khác với phương pháp chỉ dẫn đơn, mỗi tác tử trong kiến trúc này chỉ vận hành với một chỉ dẫn hẹp, chuyên biệt cho duy nhất một tác vụ nhận thức cụ thể, từ đó loại bỏ hoàn toàn hiện tượng pha loãng ngữ cảnh.

Quy trình hoạt động của hệ thống đa tác tử gồm sáu khối chức năng sau:

\begin{enumerate}

    \item \textbf{Nút Điều phối (Controller Node):} Đây là trục trung tâm quản lý toàn bộ luồng hoạt động của hệ thống. Nút Điều phối duy trì một cấu trúc dữ liệu trạng thái dùng chung, theo dõi trạng thái của từng câu hỏi qua các giai đoạn xử lý đã được định nghĩa trước. Thành phần này triển khai hai pha thử lại độc lập: Pha 1 dành cho việc sinh thân câu hỏi (nếu Tác tử Phân loại từ chối cấp độ nhận thức của một câu hỏi, câu hỏi đó sẽ được trả về Tác tử Sinh câu hỏi để tái tạo) và Pha 2 dành cho việc sửa chữa phương án trả lời (nếu Tác tử Học sinh không thể giải đúng câu hỏi, câu hỏi đó sẽ được chuyển tới Tác tử Hiệu chỉnh). Một cải tiến quan trọng của kiến trúc này là cơ chế tiến triển cưỡng bức: sau một số vòng lặp thử lại tối đa có thể cấu hình, các câu hỏi sẽ được buộc phải chuyển sang giai đoạn tiếp theo nhằm ngăn chặn hiện tượng vòng lặp vô hạn, đồng thời các cờ chất lượng sẽ được gắn kèm để đánh dấu những câu hỏi đã phải sử dụng cơ chế tiến triển cưỡng bức.

    \item \textbf{Các Tác tử Sinh câu hỏi (Generator Agents):} Hệ thống triển khai ba tác tử sinh câu hỏi hoạt động song song, mỗi tác tử chịu trách nhiệm cho một cấp độ nhận thức theo thang đo Bloom. Mỗi tác tử nhận đoạn văn bản nguồn cùng với các mẫu tích cực (các câu hỏi đạt chuẩn từ các lần sinh thành công trước đó) và các mẫu tiêu cực (các câu hỏi đã bị từ chối qua quá trình thẩm định) nhằm định hướng việc cải thiện chất lượng qua từng vòng thử lại. Hệ thống thực hiện phân bổ trước các mã định danh tuần tự toàn cục cho mỗi câu hỏi nhằm loại bỏ chi phí đồng bộ hóa trong quá trình thực thi song song.

    \item \textbf{Tác tử Phân loại (Classifier Agent):} Tác tử này có nhiệm vụ xác minh rằng mỗi câu hỏi được sinh ra phù hợp với cấp độ nhận thức mong muốn. Điểm cải tiến then chốt của thành phần này là kỹ thuật ánh xạ mã định danh tạm thời nhằm chống thiên lệch: trước khi phân loại, các câu hỏi được xáo trộn ngẫu nhiên và gán các mã định danh tạm thời mới. Kỹ thuật này loại bỏ hiện tượng thiên lệch vị trí vốn có thể khiến mô hình ngôn ngữ đơn giản xác nhận cấp độ mong đợi dựa trên thứ tự câu hỏi thay vì phân tích nội dung thực tế. Các câu hỏi không vượt qua bước phân loại sẽ được gán trạng thái thất bại và quay trở lại Tác tử Sinh câu hỏi để được tái tạo.

    \item \textbf{Tác tử Học sinh (Student Agent):} Tác tử này mô phỏng quá trình một học sinh cố gắng giải từng câu hỏi. Tác tử hoạt động với mức độ ngẫu nhiên cao hơn (tham số nhiệt độ được nâng cao) nhằm đưa thêm nhiễu vào quá trình suy luận, giúp phát hiện các câu hỏi mơ hồ hoặc có cấu trúc kém. Nếu học sinh mô phỏng trả lời đúng, câu hỏi được xác nhận đạt chuẩn. Nếu trả lời sai, câu hỏi sẽ được gắn cờ cần sửa chữa. Tác tử này đóng vai trò như một cổng kiểm soát chất lượng thực tiễn, đảm bảo rằng các câu hỏi thực sự có thể giải được từ đoạn văn bản đầu vào.

    \item \textbf{Tác tử Hiệu chỉnh (Fixer Agent):} Tác tử này tiếp nhận các câu hỏi mà Tác tử Học sinh đã trả lời sai, cùng với lý do giải thích và các phương án đã chọn của Tác tử Học sinh. Tác tử Hiệu chỉnh chỉ được phép sửa đổi các phương án trả lời và đáp án đúng, trong khi bắt buộc giữ nguyên thân câu hỏi. Ràng buộc này ngăn chặn hệ thống đơn giản tạo ra các câu hỏi hoàn toàn mới thay vì sửa chữa câu hỏi hiện có. Sau khi sửa chữa, các câu hỏi được đưa trở lại Tác tử Học sinh để tái kiểm định, tạo thành một vòng lặp cải thiện lặp đi lặp lại. Một bộ đếm số lần sửa chữa cho từng câu hỏi được duy trì nhằm ngăn chặn số lần lặp quá mức.

    \item \textbf{Nút Hợp nhất (Merge Node):} Đây là giai đoạn lắp ráp cuối cùng, có nhiệm vụ thu thập tất cả các câu hỏi có trạng thái đạt chuẩn, chọn lọc số lượng câu hỏi mục tiêu cho mỗi cấp độ nhận thức, gán các mã định danh tuần tự cuối cùng và tạo ra bộ câu hỏi hoàn chỉnh. Các câu hỏi đã phải sử dụng cơ chế tiến triển cưỡng bức sẽ được gắn cờ trong kết quả đầu ra nhằm đảm bảo tính minh bạch.
...
\begin{itemize}
    ...
    \item \textbf{Độ bám sát phân loại nhận thức (Alignment):} Chỉ số này đo lường mức độ phù hợp giữa cấp độ nhận thức mà LLM giám khảo dự đoán và cấp độ yêu cầu ban đầu:
    \[
    \text{Alignment}(q) =
    \begin{cases}
...
Chỉ số Chất lượng phương án nhiễu (Distractor Quality) được tính như sau. Đối với mỗi câu hỏi đã được chấp nhận, LLM giám khảo đánh giá từng phương án nhiễu là hợp lệ hoặc không hợp lệ theo tiêu chuẩn cấp độ tương ứng. Gọi $v(q)$ là số phương án nhiễu hợp lệ của câu hỏi $q$, với $v(q) \in \{0, 1, 2, 3\}$. Chỉ số Distractor Quality cho mỗi cấp độ $l$ được chuẩn hóa về khoảng $[0, 1]$:
...
Ngoài các chỉ số đánh giá chất lượng câu hỏi, nghiên cứu sử dụng hai thang đo chuẩn để đánh giá hiệu suất nhận dạng ký tự quang học (OCR): Tỷ lệ lỗi ký tự (Character Error Rate - CER) và Tỷ lệ lỗi từ (Word Error Rate - WER). Cả hai chỉ số đều dựa trên khoảng cách chỉnh sửa Levenshtein, đo số phép chèn ($I$), xóa ($D$) và thay thế ($S$) tối thiểu cần thiết để biến đổi chuỗi dự đoán thành chuỗi tham chiếu:
