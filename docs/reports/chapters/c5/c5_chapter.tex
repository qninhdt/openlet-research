...
% [MODIFY]: Di chuyen label xuong sau caption
...
% [NEW]
Bảng~\ref{tab:models_qg} tóm tắt các mô hình sinh câu hỏi được sử dụng trong thực nghiệm. % [MODIFY]: Them ref cheo cho chuong
Các mô hình sLLM có kích thước tham số từ 14 tỷ, phù hợp với mục tiêu triển khai trên phần cứng phổ thông. Các mô hình thương mại được đưa vào làm đường cơ sở (baseline) để đánh giá mức độ cạnh tranh của giải pháp đa tác tử kết hợp sLLM. Thông tin chi tiết về kiến trúc và đặc điểm của từng mô hình đã được trình bày tại Chương~\ref{chap:co-so-ly-thuyet}.
...
Đối với quy trình đánh giá chất lượng bằng phương pháp LLM giám khảo (đã trình bày tại Chương~\ref{chap:phuong-phap}), nghiên cứu sử dụng Gemini 3 Flash làm mô hình giám khảo cho toàn bộ các thí nghiệm. Lựa chọn này dựa trên ba tiêu chí: (i) hiệu suất suy luận cao và ổn định trên các tác vụ đánh giá ngôn ngữ, (ii) cửa sổ ngữ cảnh đủ lớn để xử lý đồng thời đoạn văn nguồn và toàn bộ bộ câu hỏi, và (iii) chi phí hợp lý cho việc đánh giá quy mô lớn (1.500 câu hỏi mỗi cấu hình).

\subsection{Phương pháp chỉ dẫn đơn}
\label{subsec:baseline}

Để đánh giá mức độ cải thiện mà kiến trúc đa tác tử mang lại, nghiên cứu thiết lập một đường cơ sở sử dụng phương pháp chỉ dẫn đơn. Phương pháp này cấu trúc hóa toàn bộ các ràng buộc nghiệp vụ, bao gồm vai trò của mô hình, định nghĩa chi tiết của ba cấp độ tư duy theo thang Bloom, tiêu chuẩn tạo đáp án nhiễu và quy định về định dạng đầu ra, trong một chỉ dẫn duy nhất. Toàn bộ quy trình sinh câu hỏi diễn ra trong một lần gọi API duy nhất, và hệ thống nhận được kết quả là danh sách các câu hỏi của cả ba cấp độ.

Lợi thế cốt lõi của phương pháp này là tối ưu hóa chi phí tính toán: chỉ cần một lệnh gọi API duy nhất, giúp giảm số lượng token cần xử lý và duy trì độ trễ ở mức thấp. Tuy nhiên, việc nén hàng loạt các quy tắc nghiệp vụ và định nghĩa của ba cấp độ vào một không gian suy luận đơn lẻ dễ dẫn đến hiện tượng pha loãng ngữ cảnh. Ngoài ra, do thiếu cơ chế giám sát và phản hồi, các câu hỏi sinh ra có thể vi phạm các quy định sư phạm như chứa nhiều hơn một đáp án đúng, câu hỏi mơ hồ hay đáp án nhiễu quá lộ liễu. Kết quả so sánh giữa đường cơ sở này và kiến trúc đa tác tử được trình bày tại các phần tiếp theo.

% [NEW]
\section{Kết quả sinh câu hỏi}

% [NEW]
% [MODIFY]: Them in nghieng cho ten chi so
Phần này trình bày kết quả đánh giá chất lượng câu hỏi trắc nghiệm được sinh bởi các mô hình theo ba nhóm: sLLM cơ bản (không có hệ thống đa tác tử), sLLM kết hợp đa tác tử, và LLM thương mại. Ba chỉ số đánh giá chính được sử dụng là \textbf{Acceptance} (tỷ lệ câu hỏi được chấp nhận), \textbf{Alignment} (mức độ phù hợp giữa cấp độ nhận thức sinh ra và cấp độ yêu cầu), và \textbf{Distractor Quality} (chất lượng các phương án nhiễu). Các chỉ số này được đo bằng phương pháp G-Eval với thang điểm từ 0 đến 1. Kết quả được phân tích theo ba cấp độ nhận thức: Cấp 1 (Nhớ), Cấp 2 (Hiểu) và Cấp 3 (Phân tích).
...
% [NEW]
Bảng~\ref{tab:sllm_multiagent} trình bày kết quả khi cùng năm mô hình sLLM được tích hợp vào hệ thống đa tác tử, nơi các tác tử Phân loại, Học sinh và Hiệu chỉnh cùng phối hợp để nâng cao chất lượng câu hỏi.
...
% [NEW]
% [MODIFY]: Loai bo thuat ngu 'anh xa ma dinh danh tam thoi'
...
% [NEW]
% [MODIFY]: Don gian hoa, loai bo thuat ngu lam dung
...
\begin{enumerate}
    ...
    \item \textbf{Tích hợp hệ thống quản lý học tập (LMS - Learning Management System):} Phát triển API và các bộ kết nối (connector) để tích hợp hệ thống Openlet với các nền tảng LMS phổ biến như Moodle, Canvas và Google Classroom, cho phép giáo viên sử dụng công cụ sinh câu hỏi trực tiếp trong quy trình giảng dạy.
