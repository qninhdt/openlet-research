...
def parse_eval_output(output: str, num_questions: int) -> List[Dict]:
    ...
    for i in range(1, len(blocks), 2):
        ...
        try:
            ...
            if clarity_match:
                clarity_reasoning = clarity_match.group(1).strip()
                clarity_score = int(clarity_match.group(2))
...
def evaluate_questions(
    ...
    for attempt in range(max_retries):
        try:
            ...
            if not output.strip():
                print(f"Warning: LLM returned empty output, retrying...")
                last_error = Exception("LLM returned empty output")
                continue
...
def print_sample_scores(item_id: str, evaluations: List[Dict]) -> None:
    ...
    for eval_item in evaluations:
        ...
        clarity_score = eval_item.get("clarity", {}).get("score", "N/A")

        print(
            f"  Q{q_num} (L{level}): Solvability={solv_score}, Distractor={dist_score}, Alignment={align_score}, Clarity={clarity_score}"
        )
...
def process_single_job(
    ...
    Returns:
        Tuple of (item_id, evaluations or error_dict, cost)
    """
    ...
    try:
        ...
        return (item_id, result, getattr(_thread_local, "cost", 0.0))

    except Exception as e:
        return (item_id, {"error": str(e)}, getattr(_thread_local, "cost", 0.0))
...
def calculate_statistics(eval_results: List[dict]) -> Dict:
    ...
    for item in eval_results:
        ...
        if "evaluations" in item:
            for eval_item in item["evaluations"]:
                ...
                if eval_item.get("clarity", {}).get("score") is not None:
                    stats["by_level"][level]["clarity"].append(
                        eval_item["clarity"]["score"]
                    )
    ...
    for level in [1, 2, 3]:
        ...
        if level_data["clarity"]:
            avg = sum(level_data["clarity"]) / len(level_data["clarity"])
            level_stats["clarity"] = round(avg, 3)
            all_clarity.extend(level_data["clarity"])
    ...
    if all_clarity:
        stats["averages"]["overall"]["clarity"] = round(
            sum(all_clarity) / len(all_clarity), 3
        )
    ...
    for level in [1, 2, 3]:
        ...
        del stats["by_level"][level]["clarity"]
...
def main():
    ...
    for source in args.sources:
        ...
        eval_results = []
        for item_id, evaluations_or_error, item_cost in job_results:
            if (
